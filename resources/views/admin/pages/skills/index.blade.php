@extends('admin.layouts.master', ['title' => 'Skills | Burak Çetinkaya'])
@section('css')
<!-- DataTables -->
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <a href="becerilerim/ekle" class="btn btn-success float-right">Add New Skills</a>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    </section>
                                ...
                                <tbody>
                                    @if($skills)
                                        @foreach($skills as $skill)
                                            <tr>
                                                <td><i class="fa fa-{{$skill->icon}}"></i></td>
                                                <td>{{$skill->title}}</td>
                                                <td>{{$skill->content}}</td>
                                                <td>{{$skill->created_at}}</td>
                                                <td>
                                                    <div class="text-center">
                                                        <a class="btn btn-primary" href="{{url('/admin/icerikyonetimi/becerilerim/duzenle')."/".$skill->id}}"><i class="fa fa-pencil"></i><span class="ml-2 font-weight-bold">Edit Skill</span></a>
                                                        <a class="btn btn-danger" href="{{url('/admin/icerikyonetimi/becerilerim/sil')."/".$skill->id}}"><i class="fa fa-trash"></i><span class="ml-2 font-weight-bold">Delete</span></a>
                                                    </div>
                                                </td>
                                            </tr>
                                            @endforeach
